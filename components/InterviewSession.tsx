...
export const InterviewSession: React.FC<InterviewSessionProps> = ({
  ...
  const displayProviderLabel = preferences.aiProvider || defaultProvider || 'OPENAI';
  ...
  }, [deepgramKey, selectedAudioDevice, audioSource, stopDeepgram]);
  ...
  const handleGenerateResponse = useCallback(async () => {
    ...
    const provider = preferences.aiProvider || defaultProvider || 'OPENAI';
    ...
    try {
      ...
      const stream = streamInterviewResponse(provider, fullTranscript, preferences, responses, {
  ...
  const timeDisplay = remainingSeconds !== Infinity 
    ...
    : `Time Used: ${currentAccumulatedSeconds}s`;
