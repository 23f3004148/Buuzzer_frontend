...
export const ScheduleInterviewPage: React.FC<Props> = ({
  ...
  const handleCreateInterview = async () => {
    try {
      ...
      showFlash('Interview scheduled. Credits bill per minute once live (3 min grace).', 'success');
  ...
  const responseStyleSelection = responseStylePresets.includes(iResponseStyle)
    ? iResponseStyle
    : 'Custom';
      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="text-center mb-8">
          ...
          <div className="inline-flex items-center gap-2 mt-3 px-4 py-2 rounded-full bg-amber-50 border border-amber-200 text-amber-700 text-xs font-medium">
            ...
            1 Credit per minute during live interview (some min grace period)
          </div>
        </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {responseStylePresets.map((preset) => (
                      <button
                        key={preset}
                        type="button"
                        onClick={() => setIResponseStyle(preset)}
                        className={`rounded-full border px-3 py-1 text-xs font-semibold transition ${
                          iResponseStyle === preset
                            ? 'border-sky-600 bg-sky-600 text-white'
                            : 'border-slate-200 bg-white text-slate-700 hover:border-sky-300 hover:text-sky-700'
                        }`}
                      >
