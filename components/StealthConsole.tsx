...
import { getStoredToken, storeToken, listCopilotSessions } from '../services/backendApi';
...
export const StealthConsole: React.FC = () => {
  ...
  // Determine API and Socket base URLs from the environment, with fallbacks.
  const SOCKET_URL =
    ...
    const handleTranscript = (chunk: any) => {
      ...
      setTranscript((prev) => {
        ...
        if (
          last &&
          last.text === text &&
          String(last?.source || '') === String(chunk?.source || '') &&
          String(last?.ts || '') === String(chunk?.ts || '')
        ) {
          return prev;
        }
        return [...prev, { text, ts: chunk?.ts, source: chunk?.source }];
      });
    };
    ...
    const messages = [
      {
        ...
        content:
          targetTab === 'code'
            ...
            : 'You are the candidate in a live interview. Write the exact answer the candidate should speak. ' +
              'Use the provided session context (resume/job/keywords) if available. ' +
              'Do not mention AI or the prompt. Do not restate the question. ' +
              'No headings, no bullet points, no markdown. ' +
              'If the question is "introduce yourself / tell me about yourself / walk me through your resume", reply in ~8–12 short lines (each line 1 sentence) covering: headline, years, domain, 2–3 relevant projects, tech stack, measurable impact, strengths, why this role, and a closing. ' +
              'Otherwise reply in ~5–9 short lines (2–3 small paragraphs). For behavioral questions use a tight STAR flow without labeling it. For technical questions answer directly, then add a quick example/trade-off. ' +
              'If the transcript is partial, infer the complete question using the recent transcript.',
      },
