...
export const UserDashboard: React.FC<Props> = ({
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setInterviews(ints || []);
      setMeetings(mets || []);
      setDefaultResumeText(profile.resumeText || '');
      setCopilotSessions(Array.isArray(sessions) ? sessions : []);
      if (pricing && typeof pricing.minCreditPurchase === 'number') {
        setMinCreditPurchase(pricing.minCreditPurchase);
      } else {
        setMinCreditPurchase(DEFAULT_MIN_CREDIT_PURCHASE);
      }
    } finally {
      setLoading(false);
    }
  ...
  const stats = useMemo(() => {
    const upcomingCount = upcomingInterviews.length + upcomingMeetings.length;
    // Completed includes all used/completed interviews + completed mentor sessions.
    const completedOnly =
      interviews.filter((i) => normalizeStatus(i.status) === 'COMPLETED' || isInterviewUsed(i)).length +
      meetings.filter((m) => (m.status || 'SCHEDULED') === 'COMPLETED').length;
    // Expired includes anything past and not completed
    const expiredOnly = expiredInterviews.length + expiredMeetings.length;
    return {
  ...
  const resolveSummary = (
    ...
    return {
      ...
      strengths: normalizeSummaryList(data?.strengths),
      gaps: normalizeSummaryList(data?.gaps),
    ...
    return (
      <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-4 shadow-sm">
        ...
        <div className="space-y-4">
          {renderList('Topics', summary.topics)}
          {renderList('Strengths', summary.strengths)}
          {renderList('Gaps', summary.gaps)}
  ...
  const downloadSummaryPdf = (
    ...
    if (summary.strengths.length) {
      ...
      addBullets(summary.strengths);
    }
    if (summary.gaps.length) {
      addSectionHeader('Gaps');
      addBullets(summary.gaps);
    }
  ...
  const handleDeleteCopilotSession = async (session: any) => {
    const id = String(session?._id || session?.id || '');
    if (!id) return;
    const confirmed = window.confirm('Delete this session and its stored transcript/summary?');
    if (!confirmed) return;
    try {
  ...
  const handleDeleteInterview = async (interview: SavedInterview) => {
    const id = String(interview?.id || '');
    if (!id) return;
    const confirmed = window.confirm('Delete this interview and its stored answers/summary?');
    if (!confirmed) return;
    try {
  ...
  const handleDeleteMeeting = async (meeting: Meeting) => {
    ...
    const confirmed = window.confirm('Delete this mentor session and its stored transcript/summary?');
    if (!confirmed) return;
    try {
                ...
                <Card>
                  ...
                  <div className="mt-3 grid grid-cols-3 gap-3 text-center">
                    ...
                    <div className="rounded-xl bg-slate-100 p-3">
                      ...
                      <div className="text-lg font-semibold">{stats.expired}</div>
                    </div>
                  </div>
                </Card>
                <Card>
